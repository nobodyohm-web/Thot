"""Scan a skill before it is trusted, the way Hermes Agent does.

Ported from Hermes Agent's `tools/skills_guard.py` (Nous Research, MIT),
whose rules are kept intact and whose reasoning transfers exactly: a skill
is text that will be handed to a model as instructions, so an untrusted one
is an injection vector, not a document.

Thot needs this more than Hermes does. A repository can carry its own
skills in `.thot/skills/`, and the repositories Thot reads are, by
definition, the ones nobody has vouched for yet. Loading a hostile repo's
skill into the briefing would hand the attacker the prompt.

Two rule sets were extended rather than rewritten: what protected
`~/.hermes/.env` and `.hermes/config.yaml` now protects `~/.thot/` too.
"""
...
SCANNER_VERSION = "thot-skill-guard-v1"
...
def _resolve_trust_level(source: str) -> str:
    ...
    # What Thot ships is trusted by construction — it is on disk because
    # the program was installed, not because a repository asked for it.
    # Hermes spelt this "official"; both spellings are accepted so a Hermes
    # caller and a Thot caller mean the same thing.
    if normalized_source in {"builtin", "thot", "official"}:
        return "builtin"
