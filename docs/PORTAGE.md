...
| `hermes/plugins/memory/hindsight/templates.py` | SSRF de second ordre, confirmée par le panel. La suppression disait `# noqa: S310 - fixed https catalog` : le catalogue n'est pas fixe — `catalog_url()` lit `HINDSIGHT_TEMPLATES_URL` dans l'environnement, les deux récupérateurs acceptent un paramètre `url`, et `fetch_manifest` construit sa cible par `urljoin` depuis un champ du catalogue **qu'il vient de télécharger** ; un `manifest_file` absolu remplace la base entièrement, donc le serveur distant choisit la destination suivante. Garde posé au point de passage. |
| `hermes/apps/desktop/src/plugins/hermes-bots/plugin.js` | Injection HTML dans l'application de bureau. `blobMarkup` construisait l'attribut avec `JSON.stringify(name)`, qui échappe le guillemet en `\"` — une convention JavaScript que HTML n'honore pas : l'analyseur termine la valeur à ce guillemet et lit la suite comme d'autres attributs. Vérifié dans node : `bot" onload="alert(1)` produit un `onload` vivant, et le balisage passe par `dangerouslySetInnerHTML`. Échappeur d'attribut correct. **Ce finding est celui que le panel n'a jamais pu trancher** — quatre tentatives, trois délais dépassés, sur un fichier de 1 660 lignes. Jugé à la main. |
| `hermes/plugins/platforms/a2a/tools.py` (`_rpc_url`) | SSRF de **troisième ordre**, confirmée par Prime avec le raisonnement écrit : `_rpc_url` préfère l'endpoint annoncé dans la carte d'agent — un document que le **pair** sert. Aucun garde de redirection ne le voit : rien ne redirige, le pair nomme simplement la destination suivante, et il peut nommer `localhost`. L'URL de la carte est contrôlée ; la base configurée est conservée si elle ne l'est pas. La base elle-même n'est pas contrôlée : un agent apairé avec un service de son propre réseau est un déploiement, pas une attaque. |
| `hermes/plugins/platforms/a2a/tools.py` (`_resolve_peer`) | `a2a_call` accepte une **URL brute** comme nom d'agent — sa docstring le dit — et poste un message dessus. Seul `a2a_discover` avait été gardé. Contrôle déplacé au point unique par lequel les deux passent, et avec le garde **strict** : `is_safe_callback_url` autorise le loopback tant qu'aucun jeton n'est configuré — la bonne règle pour un *callback* de développement local, la mauvaise pour une URL qui arrive en argument d'outil. Sur une machine par défaut, sans jeton, un modèle sous injection aurait posté vers n'importe quel service local. |
| `hermes/utils.py` · `hermes/plugins/image_gen/openai/__init__.py` | **quatrième SSRF de la même classe**, confirmée par le panel. `_load_image_bytes` fait `requests.get(ref)` où `ref` vient de `image_url` / `reference_image_urls`, c'est-à-dire d'arguments d'outil, donc du modèle. Le corps ne revient pas au modèle — mais le texte d'erreur, si : même sans lire la réponse, un modèle sous injection apprend « y a-t-il quelque chose sur ce port ». Le garde partagé vit désormais dans `utils.py`, écrit une fois parce qu'il était nécessaire à quatre endroits. |
