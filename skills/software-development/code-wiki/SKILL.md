...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.thot/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.thot/wikis/$REPO_NAME"
...
```
Generated wiki at ~/.thot/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.thot/wikis/`. Only write into the repo when the user explicitly requests it.
